...
// Firework and Balloon Animation
let w = (c.width = window.innerWidth),
    ...
    hh = h / 2,
    opts = {
        strings: ['HAPPY', 'BIRTHDAY!', 'to You', 'Farhana'],
        charSize: 40,
        charSpacing: 50,
        lineHeight: 60,
        cx: w / 2,
...
function anim() {
    ...
    ctx.fillStyle = 'rgba(43, 10, 61, 0.8)';
...
function startCountdown() {
    const now = new Date();
    // Set target to May 25, 2025, with current time plus 10 seconds
    const targetTime = new Date(now.getTime() + 120 * 1000); // Current time + 10 seconds
    const birthday = new Date(2025, 4, 25, targetTime.getHours(), targetTime.getMinutes(), targetTime.getSeconds()); // May 25, 2025
    const timer = setInterval(() => {
        ...
        if (distance < 0) {
            clearInterval(timer);
            document.getElementById('timer').innerHTML = '<h3>Happy Birthday, Farhana!</h3>';
        }
    }, 1000);
}
